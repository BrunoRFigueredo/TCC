...
@Component({
  ...
  export class DadoRegistroComponent implements OnInit{
    ...
    ngOnInit() {
      ...
      .subscribe( data => {
        console.log(data)
          ...
          this.http.get('http://localhost:8080/api/dados/cliente', options)
          .subscribe( dado => {
            console.log(dado)
              this.dados = dado; 
       }, error => {
  ...
  OnSubmit(id){
    ...
    formData.append('dado', id);
    this.http.put('http://localhost:8080/api/dados/' + id, formData)
    .subscribe(resposta => Swal.fire("Status alterado para efetuado!","Atualize a página!","success"));
